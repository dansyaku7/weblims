...
export const TemplateSurfaceWaterDocument = React.forwardRef<HTMLDivElement, { data: any }>(
  ({ data }, ref) => {
    ...
    const visibleResults = results; // Data sudah di-filter di parent
    const isMultiColumn = regulation.startsWith('pp_22');

    return (
      // Di sini perubahannya: p-10 menjadi p-8
      <div ref={ref} className="p-8 font-serif text-black bg-white relative flex flex-col" style={{ width: '210mm', minHeight: '297mm' }}>
            ...
            <tbody>
              {visibleResults.map((param: any, index: number) => (
                <React.Fragment key={`${param.name}-${index}`}>
                  {param.category && (index === 0 || visibleResults[index-1]?.category !== param.category) && (<tr className="font-bold bg-gray-100 print:break-inside-avoid"><td colSpan={isMultiColumn ? 9 : 6} className="p-1 pl-2">{param.category}</td></tr>)}
                  <tr className="text-center print:break-inside-avoid">
